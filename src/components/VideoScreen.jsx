import React, { useRef, useState, useEffect } from 'react';
import { Pause, Play } from 'lucide-react';

const VideoScreen = ({ isActive, t, startup, lang }) => {
  const videoRef = useRef(null);
  const [isPlaying, setIsPlaying] = useState(true);
  const translate = typeof t === 'function' ? t : (key) => key;

  // Quand la startup change, on recharge la vidéo
  useEffect(() => {
    if (videoRef.current) {
      videoRef.current.load();
      if (isActive) {
        const playPromise = videoRef.current.play();
        if (playPromise !== undefined) playPromise.catch(() => setIsPlaying(false));
      }
    }
  }, [startup.id, isActive]);

  const togglePlay = () => {
    if (videoRef.current) {
      if (isPlaying) videoRef.current.pause();
      else videoRef.current.play();
      setIsPlaying(!isPlaying);
    }
  };

  // Couleurs dynamiques
  const colorClasses = {
    emerald: "bg-emerald-500",
    purple: "bg-purple-600",
    orange: "bg-orange-500",
  };
  
  const badgeColor = colorClasses[startup.color] || "bg-emerald-500";
  const textColorClass = {
    emerald: "text-emerald-300",
    purple: "text-purple-300",
    orange: "text-orange-300"
  };
  const subTextColor = textColorClass[startup.color] || "text-emerald-300";

  return (
    <div className={`absolute inset-0 transition-opacity duration-500 bg-black ${isActive ? 'opacity-100 z-10' : 'opacity-0 z-0'}`} onClick={togglePlay}>
      <video
        ref={videoRef}
        className="w-full h-full object-cover"
        src={startup.video}
        poster={startup.poster}
        loop muted playsInline
      />
      
      <div className="absolute inset-0 bg-gradient-to-b from-black/10 via-transparent to-black/90 pointer-events-none"></div>

      <div className={`absolute inset-0 flex items-center justify-center transition-opacity duration-300 ${isPlaying ? 'opacity-0 hover:opacity-100' : 'opacity-100'}`}>
        <div className="bg-white/20 backdrop-blur-md p-4 rounded-full pointer-events-none">
          {isPlaying ? <Pause fill="white" size={32} /> : <Play fill="white" size={32} className="ml-1" />}
        </div>
      </div>

      <div className="absolute bottom-0 left-0 right-0 p-6 text-white pb-12 text-left pointer-events-none">
        <div className="flex items-center gap-2 mb-2">
          <span className={`${badgeColor} text-[10px] font-bold px-2 py-0.5 rounded uppercase`}>{translate('pitch_status')}</span>
          <span className="text-xs font-medium opacity-80">{startup.vision[lang] || startup.vision.fr}</span>
        </div>
        <h3 className="text-lg font-bold mb-1 tracking-tight">{startup.name}</h3>
        <p className={`text-sm font-medium mb-3 ${subTextColor} opacity-90`}>{startup.sector[lang] || startup.sector.fr} • {startup.funding}</p>
        <p className="text-sm line-clamp-2 opacity-90 leading-snug">"{startup.pitch[lang] || startup.pitch.fr}"</p>
      </div>
    </div>
  );
};

export default VideoScreen;