...
import { 
  ...
  Award, PieChart, LineChart, Video // Ajout de l'icône Video si disponible, sinon Play
} from 'lucide-react';
...
const LandingPage = ({ onLogin, onLangChange, onNavigate, initialLang = 'fr' }) => {
  ...
  // ... (fonctions handleLangChange, nextStartup, prevStartup, openModal, useEffect inchangées) ...
  ...
  return (
    <div className="min-h-screen font-sans text-left overflow-x-hidden bg-slate-50 text-slate-900">
      ...
      {/* ... WaitlistModal et Navbar inchangés ... */}
      <WaitlistModal 
        isOpen={isModalOpen} 
      ...
      <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-md border-b border-slate-200 py-3' : 'bg-transparent py-6'}`}>
        ...
        <div className={`md:hidden overflow-hidden transition-all duration-300 ${mobileMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
           {/* ... Contenu du menu mobile existant ... */}
      ...
      {/* --- NOUVELLE SECTION EXPLICATIVE / ABOUT US --- */}
      <section className="py-20 px-6 bg-slate-900 border-y border-slate-800">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-10 md:gap-16">
          ...
          <div className="flex-1 w-full max-w-md">
            <button 
              onClick={() => onNavigate && onNavigate('about')} // Navigation vers la page About
              className="group relative w-full aspect-video rounded-2xl overflow-hidden bg-slate-800 border border-slate-700 shadow-2xl flex items-center justify-center hover:border-emerald-500/50 transition-all duration-300"
            >
              {/* Effet de fond "vidéo" stylisé */}
              ...
              <div className="relative z-10 flex flex-col items-center gap-4 group-hover:scale-105 transition-transform duration-300">
                <div className="w-16 h-16 rounded-full bg-emerald-600/20 flex items-center justify-center border border-emerald-500/30 group-hover:bg-emerald-500 group-hover:text-white transition-all text-emerald-500">
      ...
      {/* ... Reste de la page (Deal Flow, Dual Profiles, Stats, Footer) inchangé ... */}
      ...
      <section id="avantages" className="py-16 lg:py-20 px-6 bg-slate-50">
          {/* ... Contenu Avantages/Dual existant ... */}
          <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12 space-y-3">
